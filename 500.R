...
A.RESULT <-B.RESULT <- ALPHA.RESULT <- GAMMA.RESULT <-  matrix(NA,J,3)
GAMMA.BETA.RESULT <- matrix(NA,J,3)
A.RESULT[,1] <- a.true
A.RESULT[,2] <-  par$a
A.RESULT[,3] <- SE.RE[,1] 
B.RESULT[,1] <- b.true
...
ALPHA.RESULT[,3] <- SE.RE[,3] 
GAMMA.RESULT[,1] <- gamma.true
GAMMA.RESULT[,2] <-  par$gamma
GAMMA.RESULT[,3] <- SE.RE[,4] 
GAMMA.BETA.RESULT[1,] <- c(beta.true,par$beta,SE[1,5])
GAMMA.BETA.RESULT[2,] <- c(delta.true,par$delta,SE[1,6])
ESTIMATE <- rbind(A.RESULT,B.RESULT,ALPHA.RESULT,GAMMA.RESULT,GAMMA.BETA.RESULT)
colnames(ESTIMATE) <- c("True","Est","SE")
invisible(ESTIMATE)
}
...
REP <- replicate(P,sim(10))
t2 <- Sys.time()
t2-t1
write.csv(REP, file = "500.csv")
